...
while terminate_cond == false 
    global α, iter, total_time, K_bar, K_newly_added, K_removed, LB, MP_obj, con_num, newCell , LB_w, p, consec_x
                ...
                while continuePartition == true #myCounter < partitionCounter 
                    ...
                    for k in K_k
                        ...
                        if gx - hx <= delta2
                            ...
                            df_temp_k = df_constraints |> 
                            @filter(_.CELL == k && _.STAT == 1)|> DataFrame
...
println("BasicAlg_ALog_"*dataSet, "; Ins ", Ins, "; β ",β,"; Time ", total_time, "; MP_obj ", MP_obj, "; x_now ", findall(x_now.==1),"; Cells ", nrow(df_cell), "; Iter ", iter)#, "; W ", LB_w, "; Cuts ", numConv)

timesFile = open("./OutputFile/BasicAlg_ALog_"*dataSet*".txt", "a")
