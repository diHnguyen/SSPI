...
myFile = myFile = "./TestInstances/"*dataSet*"_"*Ins*".jl"
include(myFile)
include("functionGbound.jl")
include("functionHbound.jl")
include("functionPartition_wSA_V2.jl")
...
#                 println("K_bar = ", K_bar)
    ...
    #                             println("Before calling Partition")
                                ...
                                # println("ΔL ", ΔL, "; ΔU ", ΔU)
                        # arc_split, yL, yU, gL, gU, SP_L, SP_U = Partition(x_now, newCell, k, p_k, c_L, c_U, M, df_cell[k,:Y])
...
println("BasicAlg_PwSA_"*dataSet, "; Ins ", Ins, "; β ",β,"; Time ", total_time, "; MP_obj ", MP_obj, "; x_now ", findall(x_now.==1),"; Cells ", nrow(df_cell), "; Iter ", iter)#, "; W ", LB_w, "; Cuts ", numConv)

timesFile = open("./OutputFile/BasicAlg_PwSA_"*dataSet*".txt", "a")
