#this implements main_singleCut_combinedStratV1.py without sensitivity analysis used to select arc base cost for partition
#i.e., A3=1.
#branching from 44-02.
#Imports for main 
import numpy as np
import pandas as pd
...
importlib.import_module("functionArcSplit") #Use the one specific for SA
    ...
    while (len(K_bar)>0):
        ...
        if m.status == 2:
            ...
            if runningTest == False:
                if printIters == True:
                    with open(directory+'Dec2024_Output/Iter/main_singleCut_combinedStratV1_A3_1_'+density+'_'+testSet+'_'+sys.argv[2]+'.txt','a') as the_file:
                        the_file.write("I;"+str(iter)+";"+str(cur_time)+';'+str(b)+";"+str(MP_obj)+";"+str(LB_global)+";"+str(x_index)+";"+str(len(K_bar))+";"+str(newCell+1)+"\n")#+";"+str(calls_SASplit)+";"+str(actual_SASplit)+";"+str(SA_time)+"\n")
...
if runningTest == True:
    ...
    with open(directory+'./Dec2024_Output/'+'main_singleCut_combinedStratV1_A3_1_'+density+'_'+testSet+'_'+sys.argv[2]+'.txt','a') as the_file:
