...
#yes: warm start sol. 1st iter (warm start sol) splits until OC2 is satisfied. 2nd and after: partition until meet OC2 as well.
    ...
    while (len(K_bar)>0):
        ...
        if O1Flag:
            ...
            while myCounter < partitionCounter:
                ...
                # print("0. ", newCell, " K_bar ", len(K_bar), end="\t")
                # print("0. UB ", sum(p[k]*g[k] for k in range(newCell+1)), end="\t")
                # print("0. LB ", sum(p[k]*h[k] for k in range(newCell+1)))
                if not K_bar:
                    myCounter = partitionCounter
                    terminate_cond = True
