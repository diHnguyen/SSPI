#Same as Strat4, but don't call alt bound when g-h is too big or too small
#The log files now have a marker for when it's in-progress (I) vs complete (C)
#Imports for main 
import numpy as np
import pandas as pd
    ...
    while (len(K_bar)>0):
        ...
        if m.status == 2:
            ...
            if runningTest == False:
                if printIters == True:
                    with open(directory+'Dec2024_Output/Iter/main_Strat4E1_'+density+'_'+testSet+'_'+n+'_'+sys.argv[2]+'.txt','a') as the_file:
                        the_file.write("I;"+str(iter)+";"+str(cur_time)+';'+str(b)+";"+str(MP_obj)+";"+str(LB)+";"+str(x_index)+";"+str(len(K_bar))+";"+str(newCell+1)+";"+str(total_SAA)+";"+str(SAA_calls)+"\n")
            # print(df_cell)
        ...
        if O1Flag:
            ...
            while myCounter < partitionCounter:
                ...
                for k in K_bar:
                    ...
                    hx_alt = 0
                    if (gx - hx > 1.5*delta2) and (gx-hx <3*delta2):
                        start_SAA = time.time()
...
if runningTest == True:
    ...
    with open(directory+'./Dec2024_Output/main_Strat4E1_'+density+'_'+testSet+'_'+n+'_'+sys.argv[2]+'.txt','a') as the_file:
        the_file.write("C;"+str(iter)+";"+str(total_time)+';'+str(b)+";"+str(MP_obj)+";"+str(LB)+";"+str(x_index)+";"+str(len(K_bar))+";"+str(newCell+1)+";"+str(total_SAA)+";"+str(SAA_calls)+"\n")
    with open(directory+'./Dec2024_Output/Iter/main_Strat4E1_'+density+'_'+testSet+'_'+n+'_'+sys.argv[2]+'.txt','a') as the_file:
        the_file.write("C;"+str(iter)+";"+str(total_time)+';'+str(b)+";"+str(MP_obj)+";"+str(LB)+";"+str(x_index)+";"+str(len(K_bar))+";"+str(newCell+1)+";"+str(total_SAA)+";"+str(SAA_calls)+"\n")
