    ...
    while (len(K_bar)>0):
        ...
        if m.status == 2:
            ...
            if runningTest == False:
                if printIters == True:
                    with open(directory+'Dec2024_Output/Iter/main_singleCut_combined_noAltBound_noWarmStart_'+density+'_'+testSet+'_'+sys.argv[2]+'.txt','a') as the_file:
...
if runningTest == True:
    ...
    with open(directory+'./Dec2024_Output/'+'main_singleCut_combined_noAltBound_noWarmStart_'+density+'_'+testSet+'_'+sys.argv[2]+'.txt','a') as the_file:
