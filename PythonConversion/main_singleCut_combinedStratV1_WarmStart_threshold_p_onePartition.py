    ...
    while (len(K_bar)>0):
        ...
        if m.status == 2:
            ...
            if runningTest == False:
                if printIters == True:
                    with open(directory+'Dec2024_Output/Iter/main_singleCut_combinedStratV1_WarmStart_threshold_p_onePartition_'+density+'_'+testSet+'_'+sys.argv[2]+'.txt','a') as the_file:
        ...
        if O1Flag:
            ...
            if (not np.array_equal(last_x, x_now)):
                K_removed = []
            partitionCounter = 1 #In this version we partition 1 cell in K_bar only, based on max_p, before re-solving
            myCounter = 0
            # print("HERE")
            ...
            while myCounter < partitionCounter:
                ...
                ######
                myCounter = partitionCounter #In this version we partition 1 cell in K_bar only, based on max_p, before re-solving
                ######
                
                df_cell_temp = df_cell[~df_cell["CELL"].isin(K_removed)] #df_cell[df_cell['CELL'].isin(K_removed)==False]
                df_cell_temp['weights'] = df_cell_temp['PROB']*(df_cell_temp['g']-df_cell_temp['h'])
                # print(df_cell_temp)
                ...
                # p_max = df_cell_temp['PROB'].max()
                # print("!!!p_max ", p_max)
                # sys.out()
                # print("p_max ", p_max)
                # K_p_max = np.where(df_cell['PROB']==p_max)['CELL']

                
                # if iter > 115:
                #     print("Cell list ", np.where(df_cell_temp['CELL'].isin(K_w)))
                
                    # print(np.where(df_cell['PROB']==p_max)[0])
                # K_p_max = np.array(df_cell[df_cell['CELL'].isin(K_w)]['CELL'])
                # print("List of cells ", np.array(df_cell[df_cell['PROB']==p_max]['CELL']))
                # print("PROB ", np.array(df_cell[df_cell['CELL'].isin(K_p_max)]['PROB']))
                # print("K_p_max ", df_cell[np.where(df_cell['PROB']==p_max)[0][0],'CELL'])
                # sys.out()
                # partition cell w max_p first
                # if no new path is found, we continue
                # if iter > 53:
                #     sys.out
                ...
                for k in K_w:#K_bar:
                    ...
                    if gx - hx <= delta2*gx: #Used to be gx - hx <= delta2:
                        ...
                        ####################################################
                        #This is the orig version in other implementations
                        # if np.array_equal(newCell_parent_Y, yL) == False:
                        #     myCounter = partitionCounter
                        #     print("!!!!!!! FOUND NEW PATH !!!!!!!")
                        ####################################################
                        if isWarmStart == False: #If isWarmStart == False, then we haven't reached the required consecutive # iter yet to partition aggressively, so just the usual.
                            myCounter = partitionCounter
                            
                        ####################################################
                        #This is only for the version that partitions 1 cell at a time before re-solving
                        if np.array_equal(newCell_parent_Y, yL) == True: #If we haven't found a new path, the aggregated cut won't see new info so we partition again
                            myCounter = partitionCounter - 1
                            # if iter > 50:
            ...
            if myCounter != partitionCounter:
                ...
                K_newly_added = []
                # K_removed = [] #Not for single cell partition
...
if runningTest == True:
    ...
    with open(directory+'./Dec2024_Output/'+'main_singleCut_combinedStratV1_WarmStart_threshold_p_onePartition_'+density+'_'+testSet+'_'+sys.argv[2]+'.txt','a') as the_file:
