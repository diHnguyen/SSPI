...
import numpy as np
import pandas as pd
def arcSplit(x_now, arc_split, k, c_L, c_U, M, y, label,edge,origin,destination,d,Len,A3,calls_SASplit,actual_SASplit,isWarmStart):
    # global A1, A2, A3, A4, A5, edge, destination, d
    # global K_bar, K_newly_added, d, df_cell
    # print("c_L ", c_L)
    # print("k = ",k)
    label = pd.DataFrame(label, columns=["node", "label"])
    # if k == 16:
    #     print("!label")
    #     print(label)
    #     print(type(label))
    ...
    if (A3 == 0) & (isWarmStart==True):  # 0=Split using SA if possible
